...
class TestDBStorageHandler:
    ...
    def test_get_by_id(self, test_db):
        # Given
        student = Student(
            name="John", surname="Doe", degree=DegreeName.bachelor, semester=1
        )
        test_db.add(student)
        test_db.commit()
        storage_handler = DBStorageHandler(session=test_db)
    ...
    def test_update(self, test_db):
        ...
        # When
        got = storage_handler.update(
            1,
            Student(
                id=1, name="Jane", surname="Doe", degree=DegreeName.bachelor, semester=1
            ),
        )
